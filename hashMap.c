...
int* hashMapGet(HashMap* map, const char* key){
    ...
    HashLink* currentLink = map->table[tableIndex];
...
void hashMapPut(HashMap* map, const char* key, int value)
{
    ...
    if (tableIndex < 0) 
    {
       tableIndex += hashMapCapacity(map);
    }
    

    if (hashMapContainsKey(map, key)) 
    {
        int* ptrValue = hashMapGet(map, key);
        (*ptrValue) += value;
    } 
    ...
    else 
    {
        ...
        assert(newLink != NULL);
        if (map->table[tableIndex] == NULL) 
        {
           map->table[tableIndex] = newLink;
        } 
...
void hashMapRemove(HashMap* map, const char* key)
{
    ...
    if (tableIndex < 0)
    {
        tableIndex += map->capacity;
    }
    
    HashLink* currentLink = map->table[tableIndex];
    HashLink* previousLink = map->table[tableIndex];;

    if (currentLink != NULL) 
    {
        if (strcmp(currentLink->key, key) == 0) 
        {
            map->table[tableIndex] = currentLink->next;
        } 
